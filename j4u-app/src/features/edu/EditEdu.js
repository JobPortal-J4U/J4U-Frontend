...
import { deleteEducations, selectEducationById, updateEducation } from "./eduSlice";
...
//   console.log(educationId);
  ...
  const education = useSelector((state) =>
    selectEducationById(state, Number(educationId))
  );

  const dispatch = useDispatch();

  const [highSchoolName, setHighSchoolName] = useState(education?.highSchoolName);
  ...
  const onSchoolNameChanged = (e) => setHighSchoolName(e.target.value);
  const onDegreeChanged = (e) => setDegreeName(e.target.value);
  const onStartDateChanged = (e) => setStartDate(e.target.value);
  const onEndDateChanged = (e) => setendDate(e.target.value);

  const canSave = [highSchoolName,degreeName,startDate,endDate].every(Boolean) && addRequestStatus === "idle";

  const navigate = useNavigate();

  const onSaveEduClicked = () => {
    if (canSave) {
      try {
        setAddRequestStatus("pending");
        dispatch(updateEducation({ id: educationId, highSchoolName,degreeName,startDate,endDate })).unwrap();
        ...
        console.error("Failed to update the Education", err);
      } finally {
                  ...
                  <div class="container shadow-lg border-0 rounded-lg  p-3 locationForm">
                    <div class="card-header  ">
                      ...
                      <div class="d-flex flex-column  ">
                        <h3 class="text-center">Update Education</h3>
                        <img
                    ...
                    <div class="card-body my-3">
                      <form>
                        ...
                        <div class=" mb-3 col-md-6 h-100 ">
                            <label for="title">HighSchool*</label>
                            <input
                              class="form-control"
                              id="highSchoolName"
                              type="text"
                          ...
                          <div class=" mb-3 col-md-6 h-100 ">
                            <label for="title">StartDate*</label>
                            <input
                              class="form-control"
                              id="startDate"
                              type="date"
